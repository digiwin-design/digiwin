...
function moveNextArrow() {
    let distance = $shorten.position().top - $(window).height(); // 影片下方與視窗底部的距離
...
$(window).on('scroll', function() {
    let pos = $(this).scrollTop() + 5; // 因有誤差須加上偏移量
    ...
    if (flag) {
        // 依據網頁位置變更導覽列連結樣式
        let result = sectionPos.filter(function(value, index) {
