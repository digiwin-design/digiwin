...
// let hash = '';
// final
var a=location.href.split('/'),b=a.find(function(a){return a.startsWith('WB')}),c='#/'+a[a.length-1].replace(/(.html|.htm)/g,''),d=document.querySelector('.header-box header');null===d||new Vue({el:'#submenu',data:{results:[],currentPage:'',activeIndex:0,isActive:!1},computed:{submenu:function a(){return document.querySelector('.page-submenu')}},created:function a(){d.insertAdjacentHTML('beforeend','<div id="submenu"></div>'),this.getData()},mounted:function b(){var a=this;document.querySelector('body').addEventListener('touchstart',function(b){return a.closeDropdownHandler(b)})},beforeDestroy:function a(){document.querySelector('body').removeEventListener('touchstart'),window.removeEventListener('scroll',this.scrollHandler),window.removeEventListener('resize',this.resizeHandler)},methods:{// 取得選單內容
...
setActiveIndex:function b(){var a=this;this.results[0].menus.some(function(b,d){return b.url===c?(a.activeIndex=d,a.setCurrentPage(b.text),!0):b.dropdowns?b.dropdowns.some(function(b){if(b.url===c)return a.activeIndex=d,a.setCurrentPage(b.text),!0}):void 0})},// 設定標題文字
...
linkTo:function i(a){var b=this,c=a.url,d=a.target,e=a.anchor;if(d)window.open(''+c,d);else if(c&&!e)window.open(''+c,'_self'),this.setPageInfo(c);else if(!c&&e){var f=$(e).offset().top,g=document.querySelector('html').classList.contains('production')&&$('.page-submenu').outerHeight(),h=g?f-g:f;$('html, body').animate({scrollTop:h}),this.closeDropdownHandler()}else c&&e&&(// 已棄用，應使用各頁面的 router.beforeEach 處理 (如製造業)
// 切換頁面後延遲兩秒移動至錨點
window.open(''+c,'_self'),setTimeout(function(){var a=$(e).offset().top,c=$('.page-submenu').outerHeight();$('html, body').animate({scrollTop:a-c}),b.closeDropdownHandler()},2e3),this.setPageInfo(c))},// 設定頁面資訊
setPageInfo:function b(a){'javascript:;'===a||(c=a,this.setActiveIndex())},openDropdown:function b(a){$(a.target).parents('li').addClass('is-open').siblings().removeClass('is-open')},closeDropdownHandler:function b(a){a&&1===$(a.target).parents('.page-submenubox').length||$('.page-submenu-list > li').removeClass('is-open')},resizeHandler:function c(){var a=this.submenu.offsetHeight,b=document.querySelector('main');null===b||(b.style.paddingTop=50<a?'50px':0)}},template:'\n            <div class="page-submenubox" :class="{active:isActive}">\n                <div class="page-submenu">\n                    <div class="container">\n                        <template v-for="result in results">\n                            <h2>{{setTitle(result.title)}}</h2>\n                            <ul class="page-submenu-list">\n                                <li v-for="(menu, index) in result.menus">\n                                    <span :class="{active:index===activeIndex}">\n                                        <a :href="menu.url" @click.prevent="linkTo(menu)" @touchstart="openDropdown">{{menu.text}}</a>\n                                    </span>\n                                    <ul v-if="menu.dropdowns" class="page-submenu-dropdown">\n                                        <li v-for="dropdown in menu.dropdowns">\n                                            <a :href="dropdown.url" @click.prevent="linkTo(dropdown)">{{dropdown.text}}</a>\n                                        </li>\n                                    </ul>\n                                </li>\n                            </ul>\n                        </template>\n                    </div>\n                </div>\n            </div>\n        '})})();
